...
- maldoc in PDFが気になったので色々調べてみた。
- maldoc in PDFを改造してみる。
- 改造結果を確認してみる。
...
前頭にゴミ文字列があっても正常に開ける、となると次に気になるのは何バイトまでゴミ文字列を挿入できるかというところです。
...
この推測が正しいとするとMHTMLファイルの先頭に埋め込めるPDFデータは4095バイト以内であればOKということになります。

# maldoc in PDF 作成

ただし、ExcelやWordから作成したPDFは4095バイトを余裕で超えてしまうので、検体からPDF部分のデータを拝借します。
